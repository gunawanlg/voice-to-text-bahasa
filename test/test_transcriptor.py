...
class TranscriptorTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        np.random.seed(42)
        cls._X = np.random.rand(5, 1000, 39) # 39 MFCC features,
                                             # 1000 sequence length
                                             # 1 input example
        
        cls._y = [
            "adam, set, enos, kenan, mahalaleel, yared, henokh, metusalah, lamekh, nuh, sem, ham dan yafet.",
    ...
    def test_fit_predict_with_y(self):
        transcriptor = ASRTranscriptor(self.model, CharMap.IDX_TO_CHAR_MAP)
    ...
    def test_fit_predict_low_memory_with_y(self):
        transcriptor = ASRTranscriptor(self.model, CharMap.IDX_TO_CHAR_MAP, True)
        transcriptor.fit(self._X, self._y)
        y_pred = transcriptor.predict(self._X)
