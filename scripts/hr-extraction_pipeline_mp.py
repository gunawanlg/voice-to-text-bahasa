...
if __name__ == '__main__':
    ...
    non_extracted_mp3s = [
        f"{k.replace('npz', 'mp3')}" for k, v in file_nonavailability_dict.items() if v
    ]
    ...
    for i, b in zip(range(cpus - 1), batches):  # don't use all cores
        p = multiprocessing.Process(target=extraction_pipeline_batch,
                                    args=[i, b, norm_feature_extractor])
        jobs.append(p)
        p.start()
