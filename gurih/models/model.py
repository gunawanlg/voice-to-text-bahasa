...
class Seq2SeqModel(_BaseModel):
    ...
    def _create(self):
        # Encoder parts
        encoder_inputs = Input(shape=(None, self.vocab_len))
        encoder = LSTM(self.latent_dim, return_state=True)
        encoder_outputs, state_h, state_c = encoder(encoder_inputs)
        encoder_states = [state_h, state_c]

        decoder_inputs = Input(shape=(None, self.vocab_len))
        decoder = LSTM(self.latent_dim, return_sequences=True, return_state=True)
        decoder_outputs, _, _ = decoder(decoder_inputs, initial_states=encoder_states)
        decoder_outputs = Dense(self.vocab, activation='softmax')(decoder_outputs)
